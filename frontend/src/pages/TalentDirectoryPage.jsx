    ...
    <div className="min-h-[calc(100vh-72px)] bg-premium-light py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto flex flex-col gap-6">
        <div className="border-b border-gray-200/80 pb-4">
          <span className="text-xs font-extrabold uppercase tracking-widest text-emerald-600">Verified Roster</span>
          <h1 className="text-2xl font-black text-premium-dark mt-0.5">Talent Directory</h1>
        </div>

        {isLoading ? (
          <div className="text-center py-12 text-xs font-bold tracking-widest text-gray-400 animate-pulse">
            Querying Talent Nodes...
          </div>
        ) : talent.length === 0 ? (
          <div className="text-center py-12 text-sm font-bold text-gray-500 bg-white rounded-2xl border border-gray-100 shadow-sm">
            No talent profiles found in the database.
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {talent.map((user) => (
              <GlassCard key={user._id} className="flex flex-col gap-4 border-gray-200/80">
                <div className="flex items-center gap-4 border-b border-gray-100 pb-4">
                  <div className="w-12 h-12 rounded-full bg-gray-200 flex items-center justify-center font-black text-gray-500">
                    {user.name ? user.name.charAt(0) : 'U'}
                  </div>
                  <div>
                    <h3 className="text-base font-bold text-premium-dark">{user.name}</h3>
                    <span className="text-[10px] font-bold uppercase text-gray-400">{user.category || 'Specialist'}</span>
                  </div>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm font-black text-amber-500">★ {user.rating || 'New'}</span>
